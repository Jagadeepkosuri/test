...
class NeuDataOpsAzureCosmosDBHook(BaseHook):
    """
    ...
    def get_database(self,database_name):
        ...
        existing_database = list(self.get_conn().query_databases({
    ...
    def does_collection_exist(self, dbproxy,collection_name):
        """
        ...
        existing_container = list(dbproxy.query_containers(
            get_database_link(self.default_database_name), {
                "query": "SELECT * FROM r WHERE r.id=@id",
                "parameters": [
    ...
    def create_collection(self, dbproxy,collection_name,partition_key):
        """
        ...
        # to create it twice
        existing_container = list(dbproxy.query_containers(
            get_database_link(self.__get_database_name(self.default_database_name)), {
                "query": "SELECT * FROM r WHERE r.id=@id",
                "parameters": [
                    {"name": "@id", "value": collection_name}
                ]
    ...
    def create_database(self):
        ...
        """
        if self.default_database_name is None:
            raise AirflowBadRequest("Database name cannot be None.")
        ...
        existing_database = list(self.get_conn().query_databases({
            "query": "SELECT * FROM r WHERE r.id=@id",
            "parameters": [
                {"name": "@id", "value": self.default_database_name}
            ]
        }))
        logging.info("DB Name 3:" + str(self.default_database_name))
        logging.info("DB Name 4:" + str(existing_database))

        # Only create if we did not find it already existing
        if len(existing_database) == 0:
            return self.get_conn().create_database({"id": str(self.default_database_name)})
        else:
            return None
    ...
    def get_database_client(self, database):
        ...
        return DatabaseProxy(self.connection, id_value)
